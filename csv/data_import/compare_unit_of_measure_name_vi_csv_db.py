...
# -*- coding: utf-8 -*-
"""
So sánh name_vi: data/Danh_sach_don_vi_tinh.csv ↔ dbo.unit_of_measure (Đơn vị tính).

Chỉ dùng cột NameVi trong CSV (các cột khác không dùng cho so khớp).

Chuẩn hóa: trim + lower. "-", "n/a", rỗng → bỏ qua.

In:
  - Có trên CSV, không có trên DB
  - Có trên DB, không có trên CSV (kèm unit_of_measure_code nếu có)

  python3 compare_unit_of_measure_name_vi_csv_db.py
  python3 compare_unit_of_measure_name_vi_csv_db.py --csv /path/to/Danh_sach_don_vi_tinh.csv
"""
...
def pick_name_vi_column(fieldnames: List[str]) -> str:
    fields = {h.strip(): h for h in (fieldnames or []) if h}
    for k in fields:
        kl = k.lower().replace(" ", "")
        if kl == "namevi" or kl == "name_vi":
            return fields[k]
    for h in fieldnames or []:
        if not h:
            continue
        hl = h.strip().lower()
        if "tiếng" in hl and "việt" in hl:
            return h
    raise ValueError(f"Không tìm thấy cột NameVi. Header: {fieldnames!r}")
...
def load_db_name_vi(cur: "pyodbc.Cursor") -> Tuple[Set[str], Dict[str, List[Tuple[str, str]]]]:
    cur.execute(
        """
        SELECT LTRIM(RTRIM(ISNULL(unit_of_measure_code, N''))),
               LTRIM(RTRIM(ISNULL(name_vi, N'')))
...
def main() -> int:
    ...
    print(f"name_vi duy nhất (CSV): {len(csv_norms)}")
    print(f"name_vi duy nhất (DB, có nội dung): {len(db_norms)}")
    print()

    print(f"=== Có trên CSV, chưa có trên DB ({len(only_csv)}) ===")
    if only_csv:
        for k in only_csv:
    ...
    if only_db:
        for k in only_db:
            for code, nvi in db_by_norm.get(k, []):
